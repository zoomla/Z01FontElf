...
#set title
# main.title("字体计数软件")
main.title("Z01 FontElf-逐浪字体精灵v1.0 逐浪字库出品 http://f.ziti163.com 字库授权与订制热线：0791-86161900、13177777714")  # 程序名称
#window size
main.geometry('')
#prepare font
...
#text label + button
input_lbl = Label(frame, text="选取或拖到exe图标：　　　", font=text_font)
input_lbl.grid(column=0, row=0, sticky=E)
btn = Button(frame, text ='从本地选取字体文件', font=text_font, command = lambda:open_file(None),background='blue',fg='#fff' ) #just send None as no drag and drop file
...
font_txt_lbl = Label(frame, text="当前检测字体：", justify="right", font=text_font)
...
#use last row
notice_oldname = Label(frame, text="＊旧称「3500 字常用汉字表」,逐浪字库出品，基于CJK-character-count开源项目。", justify="right", font=text_font).grid(column=3, row=last_row, sticky=E, columnspan=3)
...
btn = Button(frame, text ='♛字體網-业界知识门户', font=text_font, command = openLink2,background='red',fg='#ffffff' ) #just send None as no drag and drop file
...
# notice_copyright = Label(frame, text="逐浪字库出品，基于CJK-character-count开源项目", justify="right", font=text_font).grid(column=3, row=last_row, sticky=E, columnspan=3)
...
def open_file(filename_arg):
    ...
    if is_a_font:
        ...
        font_name.set("无")
        messagebox.showwarning(title="非字体", message="这不是一个标准字体文件。")
    else:
        #unknown error
        font_name.set("无")
